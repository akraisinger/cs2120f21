...
/- The conjecture z = w is true via the symmetric property of equality. -/
...
/- When dealing with ∀ statements, you must make an assumption
that if you are able to prove it true for an abitrary object
of that type, then it is true for all objects of that nature. -/
...
in the following expression: (∀ t, P t). 
-/
